...
func main() {
	...
	healthEndpoint = kit_svc.NewTokenBucketLimitterWithBuildIn(ratebucket)(healthEndpoint)
	healthEndpoint = kitzipkin.TraceEndpoint(zipkinTracer, "health-endpoint")(healthEndpoint)

	//身份认证Endpoint
	authEndpoint := kit_svc.MakeAuthEndpoint(svc)
	...
	//创建http.Handler
	r := kit_svc.MakeHttpHandler(ctx, endpts, zipkinTracer, logger)
